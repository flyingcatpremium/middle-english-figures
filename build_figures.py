...
.controls{
  ...
  /* always allow internal scroll so the new-entry form is never clipped */
  max-height: calc(100vh - 220px);
  overflow-y: auto;
  flex-shrink: 0;
}
.controls::-webkit-scrollbar{width:8px}
.controls::-webkit-scrollbar-thumb{background:#cbbe9f88; border-radius:4px}
.controls::-webkit-scrollbar-thumb:hover{background:#cbbe9f}
...
.card .meta .sub{
  font-size:12px; color:var(--ink-soft); margin-top:1px; font-family:'Inter',sans-serif;
  white-space:nowrap; overflow:hidden; text-overflow:ellipsis;
}
.card .meta .age{
...
.card .meta .role{
  ...
  white-space:nowrap; overflow:hidden; text-overflow:ellipsis;
  display:none; /* 隐藏第四行（已编辑/原始/用户添加），让卡片更紧凑统一 */
}
...
.hero .portrait{cursor:pointer; transition:outline-color .15s ease}
.hero .portrait.dragover{outline:3px dashed var(--berkeley-gold); outline-offset:-3px; background:#fffbeb}
.hero .portrait .drophint{
  position:absolute; top:0; left:0; right:0; bottom:0;
  display:none; place-items:center; flex-direction:column; gap:6px;
  background:rgba(255,251,235,.9); color:var(--berkeley-blue);
  font-family:'Inter',sans-serif; font-size:13px; font-weight:600;
  letter-spacing:.5px; pointer-events:none;
}
.hero .portrait.dragover .drophint{display:flex}
.hero .portrait .drophint .icn{font-size:36px; color:var(--berkeley-gold)}
...
/* 简介 / 作品 / 事件 / 关系 / 备注 等栏目里粘贴/插入的图片：统一为书本大小（约 5:7 竖版） */
.body .editable img{
  display:block;
  width:180px;
  height:252px;
  ...
  border-radius:4px;
  border:1px solid var(--rule);
  box-shadow:0 6px 16px rgba(28,24,18,.18), 0 2px 4px rgba(28,24,18,.10);
  background:#efe6d2;
}
.body .editable a img{ margin:14px auto; }
.body .editable img.icon{
  width:auto; height:1.1em; display:inline; margin:0 .15em;
  box-shadow:none; border:none; border-radius:0; background:transparent;
}
@media (max-width: 920px){
  .body .editable img{ width:150px; height:210px; }
}
.body h2{
  font-family:'EB Garamond',serif;
...
/* allow the controls strip itself to scroll internally when the form makes it taller than the sidebar */
aside.directory:has(.adddialog.open) .controls{
  max-height: calc(100vh - 160px);
  overflow-y: auto;
}
/* fallback for browsers without :has() — give the dialog its own internal scroll */
.adddialog.open{
  max-height: calc(100vh - 200px);
  overflow-y: auto;
}
/* sticky save bar so 保存并打开 stays reachable as user scrolls inside the form */
.adddialog .actions{
  position: sticky;
  bottom: -10px;
  margin: 8px -12px -10px;
  padding: 8px 12px 10px;
  background: linear-gradient(180deg, transparent, #fffbeb 30%);
  z-index: 1;
}
.adddialog .head{
  display:flex; justify-content:space-between; align-items:center;
...
function renderProfile(p){{
  ...
  /* user entries: editable kind via <select>; seed entries: static badge */
  const kindTag = isUserId(p.id)
    ? `<select id="entryKindSel" class="kindbadge ${{p.kind||'figure'}}" style="font-size:11px;padding:2px 9px;border:none;cursor:pointer;font-weight:700;letter-spacing:.5px">${{Object.entries(KIND_LABEL).map(([k,v])=>`<option value="${{k}}" ${{k===(p.kind||'figure')?'selected':''}}>${{v[0]}} · ${{v[1]}}</option>`).join('')}}</select>`
    : `<span class="kindbadge ${{p.kind||'figure'}}" style="font-size:11px;padding:2px 9px">${{kindLbl[0]}} · ${{kindLbl[1]}}</span>`;
  ...
  profileEl.innerHTML=`
    <div class="hero">
      <div class="portrait" id="portrait" style="overflow:hidden">
        ...
        <div class="drophint"><div class="icn">⬆</div>放下图片即可</div>
        <div class="photoedit" contenteditable="true" data-field="photo" data-placeholder="拖入图片，或点击选择，或粘贴 URL / photos/ 文件名">${{p.photo||""}}</div>
      </div>
      <div>
  ...
  /* ---- portrait drag-drop / click-to-pick ---- */
  const portrait=profileEl.querySelector('#portrait');
  if(portrait){{
    portrait.addEventListener('dragenter', e=>{{ if(e.dataTransfer && [...(e.dataTransfer.types||[])].includes('Files')){{ e.preventDefault(); portrait.classList.add('dragover'); }} }});
    portrait.addEventListener('dragover',  e=>{{ if(e.dataTransfer && [...(e.dataTransfer.types||[])].includes('Files')){{ e.preventDefault(); portrait.classList.add('dragover'); }} }});
    portrait.addEventListener('dragleave', e=>{{ if(e.target===portrait || !portrait.contains(e.relatedTarget)) portrait.classList.remove('dragover'); }});
    portrait.addEventListener('drop', e=>{{
      e.preventDefault(); portrait.classList.remove('dragover');
      const f=e.dataTransfer && e.dataTransfer.files && e.dataTransfer.files[0];
      if(f) ingestImageFile(f);
    }});
    portrait.addEventListener('click', e=>{{
      /* don't intercept clicks inside the contenteditable URL bar */
      if(e.target.closest('.photoedit')) return;
      const inp=document.createElement('input');
      inp.type='file'; inp.accept='image/*';
      inp.addEventListener('change',()=>{{ if(inp.files[0]) ingestImageFile(inp.files[0]); }});
      inp.click();
    }});
  }}
  ...
  /* kind selector for user entries — switching kind re-renders sections per kind */
  const kindSel=profileEl.querySelector('#entryKindSel');
  if(kindSel){{
    kindSel.addEventListener('change',()=>{{
      const arr=loadUserEntries();
      const idx=arr.findIndex(x=>x.id===currentId);
      if(idx<0) return;
      arr[idx].kind=kindSel.value;
      saveUserEntries(arr);
      renderList(); renderProfile(entryById(currentId)); flashSaved();
    }});
  }}
...
/* ---------- Image ingest (drop / click-to-pick) ---------- */
...
function ingestImageFile(file){{
  if(!file || !/^image\//.test(file.type)){{ alert('请拖入图片文件（PNG / JPG / WEBP / GIF）。'); return; }}
  setSavingState('saving');
  resizeImageToDataUrl(file, 600, 800, 0.85).then(dataUrl=>{{
    if(isUserId(currentId)){{
      const arr=loadUserEntries();
      const idx=arr.findIndex(x=>x.id===currentId);
      if(idx<0) return;
      arr[idx].photo=dataUrl;
      saveUserEntries(arr);
    }} else {{
      const ov=loadOverrides(currentId);
      ov.photo=dataUrl;
      saveOverrides(currentId, ov);
    }}
    const p=entryById(currentId);
    renderProfile(p); flashSaved();
  }}).catch(err=>{{
    console.error(err);
    alert('读取图片失败：'+(err.message||err));
    setSavingState('');
  }});
}}
...
function buildBlankEntry(kind, name){{
  const nm = (name||'').trim() || '新条目';
  /* 简单从 "Geoffrey Chaucer" 这种名字里拆 surname/given */
  const parts = nm.split(/\s+/);
  const given   = parts.length>1 ? parts.slice(0,-1).join(' ') : nm;
  const surname = parts.length>1 ? parts[parts.length-1] : nm;
  /* initials：取前两个字符（中文取首字，英文取首字母大写） */
  const initials = (function(){{
    const ascii = nm.match(/[A-Za-z]+/g);
    if(ascii && ascii.length){{
      return (ascii[0][0] + (ascii[1]?ascii[1][0]:'')).toUpperCase();
    }}
    return nm.slice(0,2);
  }})();
  return {{
    ...
    surname, given,
    era: '', role: '',
    birthYear: null, deathYear: null, year: null,
    lifespan: '',
    title: '', titleZh: '',
    affiliation: '', affiliationZh: '',
    location: '', eventType: '', placeType: '',
    language: '', termType: '',
    specialties: [],
    initials,
    photo: "", photoPos: "50% 30%", thumbPos: "50% 35%", thumbZoom: "1",
    sourceUrl: '',
    bio: '', works: '', events: '', relations: '', notes: '',
  }};
}}

addBtn.addEventListener('click', ()=>{{
  /* persist any pending edits on the current profile first */
  if(profileEl.querySelector('[contenteditable="true"]')) persist();

  /* 只让用户粘贴/输入一个名字，立即创建条目 */
  const raw = window.prompt('粘贴或输入条目名称（中文/英文均可，例如 "Geoffrey Chaucer" 或 "乔叟"）：','');
  if(raw===null) return;            /* 用户取消 */
  const name = raw.trim();
  if(!name) return;                 /* 空字符串当作取消处理 */

  const obj = buildBlankEntry('figure', name);
  const arr = loadUserEntries(); arr.push(obj); saveUserEntries(arr);

  /* clear filters/search so the new entry isn't hidden */
  state.q="";   if(qEl)qEl.value="";
  state.kind=""; state.era=""; state.role="";
  if(kindChips) kindChips.querySelectorAll('.chip').forEach(c=>c.classList.toggle('active', !c.dataset.kind));
  if(eraChips)  eraChips .querySelectorAll('.chip').forEach(c=>c.classList.toggle('active', !c.dataset.era));
  if(roleChips) roleChips.querySelectorAll('.chip').forEach(c=>c.classList.toggle('active', !c.dataset.role));
