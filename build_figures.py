...
function openAddDialog(){{
  ...
  /* scroll the dialog into view inside the controls panel */
  setTimeout(()=>{{
    addDialog.scrollIntoView({{behavior:'smooth', block:'start'}});
    addName.focus();
  }}, 30);
}}
...
addSave.addEventListener('click', ()=>{{
  ...
  /* clear search box so the new entry isn't hidden by a stale query */
  if(state.q){{ state.q=""; qEl.value=""; }}
  /* reset any active filter that would hide the new entry */
  ...
  if(state.era && state.era!==obj.era){{
    state.era="";
    eraChips.querySelectorAll('.chip').forEach(c=>c.classList.toggle('active', !c.dataset.era));
  }}
  if(state.role && state.role!==obj.role){{
    state.role="";
    roleChips.querySelectorAll('.chip').forEach(c=>c.classList.toggle('active', !c.dataset.role));
  }}
  currentId=obj.id;
